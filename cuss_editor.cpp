...
// draw_line() and draw_box() actually add them to the interface's background.
void draw_line(interface &edited, int x1, int y1, int x2, int y2);
void draw_box (interface &edited, int x1, int y1, int x2, int y2);
// temp_line() and temp_box() just draw them temporarily
  ...
  while (!exit_program) {
    ...
    if (argc >= 2) {
      ...
      if (!file_exists(filename)) {
        ...
        return 1;
      } else if (!edited.load_from_file(filename)) {
          std::cout << "File '" << filename <<
                       "' exists, but could not be loaded.";
          return 1;
      }
    } else {
      exit_program = starting_window(edited);
    }
    bool done = exit_program;
    while (!done) {
      if (dm == DM_DRAW)
      ...
      } else if (dm == DM_BOX || dm == DM_ELEMENT || dm == DM_DELETE ||
                 dm == DM_FIXLINES) {
...
// End of "if (movex != 0 || movey != 0)" block
...
// Advance through the line-drawing symbols.
            switch (pen.symbol) {
...
// Paste the drawing from the clipboard, if it's not empty.
            if (!clipboard.drawing.empty()) {
// The clipboard is just a map of Points to glyphs; so we push the glyph to BG
// (our background drawing element), offsetting each point by the current cursor
// position.  This is safe even if the resulting point is out of bounds!
              for (std::map<Point,glyph>::iterator it = clipboard.drawing.begin();
                   it != clipboard.drawing.end();
                   it++) {
                edited.set_data("BG", it->second,
                                it->first.x + posx, it->first.y + posy);
              }
            } else {
...
/* The 'x' key has two functions:
 * If an element is selected, we delete it (after asking for confirmation)
 * If an element is NOT selected, we erase the character under the cursor.
 */
            if (sel) {
              if (popup_getkey("Really delete %s?", sel->name.c_str()) == 'Y') {
...
/* Pressing enter has different behavior depending on the current draw mode.
 * In standard mode, we start creating an element; in drawing mode, we move down * a line and reset x to the buffered value; for all other modes, we "finish"
 ...
 */
            switch (dm) {
              ...
              case DM_ELEMENT: {
                ...
                dm = DM_NULL;
                switch (menu("Element type:",
                             "Drawing", "Text", "List", "Text Entry",
...
// Escape cancels whatever we were doing.
            if (sel) {
...
// Hitting enter works even if we're in search mode.
    if (ch != '\n' && searchmode) {
      ...
      if (ch == 'n' || ch == 'N') {
        ...
        if (name != "") {
          int x = int_input_popup("X dim:"),
              y = int_input_popup("Y dim:");
          if (x > 0 && y > 0) {
            done = true;
    ...
    switch (ch) {
      ...
      case 'A': {
        ...
        if (used) {
          ...
          errormes << "<c=red>ERROR: " << key_name(key) <<
                      " is already bound to " << action_name(used->act) << "!";
      ...
      if (selected && ch == '{') {
        ...
        if (ch == ']') {
          ...
          if (!selected->name.empty()) {
              selected->name = selected->name.substr(0,
                                                     selected->name.size() - 1);
              i_ele.set_data("e_elelist", edited.element_names());
          }
        } else if ((ch >= 'a' && ch <= 'z') ||
                   (ch >= 'A' && ch <= 'Z') ||
        ...
        if (selected && selected->type() == ELE_TEXTBOX) {
        ...
        } else if (selected && (selected->type() == ELE_LIST ||
                                selected->type() == ELE_MENU   )) {
          ...
          if (ch == 'j' || ch == 'J' || ch == '2' || ch == KEY_DOWN) {
          ...
          } else if (ch == 'k' || ch == 'K' || ch == '8' || ch == KEY_UP) {
            i_ele.add_data("e_list_values", -1);
          ...
          } else if (list_array && pos >= 0 && pos < list_array->size() &&
                     (ch == 'e' || ch == 'E'))
          ...
          } else if (ch == '>' && list_array &&
                     pos >= 0 && pos < list_array->size() - 1) {
            ...
            i_ele.set_data("e_list_values", pos + 1);
                     ...
                     pos >= 1 && pos < list_array->size()) {
            std::string buff = (*list_array)[pos];
            (*list_array)[pos] = (*list_array)[pos - 1];
            (*list_array)[pos - 1] = buff;
            i_ele.set_data("e_list_values", pos - 1);
          }
        } // Editing "e_list_values"
      } 
    }
  } while (!done);
}
